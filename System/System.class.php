...
class System {
  ...
  private function initTheme() {
    ...
    if ($settings['debug']) {
      ...
      $this->twig->addExtension(new Debug\TwigKrumoExtension());
    }
  }
  ...
  public function handleErrors($errno, $errstr, $errfile, $errline) {
    try {
      throw new Exception\PhpError($errno, $errstr, $errfile, $errline, $this, TRUE);
    }
  ...
  private function _render($parent) {
    ...
    // Theme the item.
    $parent->_rendered = $this->theme($parent->getTemplates(), $parent->getBuildData());
  }
}
